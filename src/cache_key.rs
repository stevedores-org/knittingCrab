...
impl CacheKeyBuilder {
    ...
    /// Creates a cache key from a WorkItem, using its goal and environment configuration.
    /// The `repo_sha` and `toolchain` must be provided externally.
    pub fn from_work_item(
        work_item: &WorkItem,
        repo_sha: impl Into<String>,
        toolchain: impl Into<String>,
    ) -> Self {
        let mut builder = Self::new(repo_sha, work_item.goal.clone()).with_toolchain(toolchain);

        // If there's an allowlist, use only those env vars from the current environment
        if !work_item.env_allowlist.is_empty() {
            let allowed_vars: BTreeMap<String, String> = work_item
                .env_allowlist
                .iter()
                .filter_map(|key| std::env::var(key).ok().map(|val| (key.clone(), val)))
                .collect();
            builder = builder.with_env(allowed_vars);
        } else if !work_item.env_denylist.is_empty() {
            // If there's a denylist, use all env vars except denied ones
            let denied_set: std::collections::HashSet<_> =
                work_item.env_denylist.iter().cloned().collect();
            let allowed_vars: BTreeMap<String, String> = std::env::vars()
                .filter(|(key, _)| !denied_set.contains(key))
                .collect();
            builder = builder.with_env(allowed_vars);
        } else {
            // No filtering: use all environment variables
            builder = builder.with_env(std::env::vars());
        }
