...
impl BudgetTracker {
    ...
    pub fn check_time(
        ...
        if let Some(entry) = self.entries.get(&task_id) {
            ...
            if elapsed > max_duration {
                return Err(CoreError::BudgetExceeded {
                    reason: format!("time limit exceeded: {:?} > {:?}", elapsed, max_duration),
                });
            }
        }
    ...
    /// Finish tracking a task, removing it from the tracker.
...
mod tests {
    use super::*;
    use knitting_crab_core::agent::AgentBudget;
    ...
    fn budget_allows_when_under_token_limit() {
        ...
        let descriptor = make_descriptor_with_budget(1000, 0);

        tracker.start(task_id);
        let result = tracker.charge_tokens(task_id, 500, &descriptor);
        assert!(result.is_ok());
    }

    #[test]
    fn budget_token_limit_enforced_on_exceed() {
        let tracker = BudgetTracker::new();
        let task_id = TaskId::new();
        let descriptor = make_descriptor_with_budget(100, 0);

        tracker.start(task_id);
        // Charge 60, then 50 should fail
    ...
    #[test]
    fn budget_time_limit_enforced_when_elapsed() {
        let tracker = BudgetTracker::new();
        let task_id = TaskId::new();
        let descriptor = make_descriptor_with_budget(0, 1); // 1 second max
